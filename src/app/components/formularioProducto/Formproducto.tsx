...
import { SForm, Slabel, SField, SErrorMessage ,Cbutton,Sbutton,DivSend,Ddiv,Img2Edit,Ibutton,DdivLarge,DivAdj,Cdiv,SAsterisk} from './Formproducto.style';
...
const Formproduct: React.FC<ProductoSend>=({id,productoS,onAccept}) => {
    const initialValues = {
        name: productoS.name,
    ...
    const handleSubmit = async (ProductoIDS:number,values:Object) => {
        try {
          values = { ...values, thumbnail_url: newImageUrl };
          const JSONval=JSON.stringify(values)
          console.log(JSONval)
          const response =await axios.put('http://localhost:3000/api/modificaproducto?id='+ProductoIDS,JSONval)
